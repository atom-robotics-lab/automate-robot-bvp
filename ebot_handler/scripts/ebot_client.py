...
class PerceptionActionClient:
    '''A class to create and manage an action client of the perception action server'''

    def __init__(self):
        # initialize perception action client
        self.Perception_client = actionlib.SimpleActionClient('Perception_action_server', PerceptionAction)
        rospy.loginfo("ebot_perception: waiting for Perception server ...")

        # wait for the perception action server to start
        self.Perception_client.wait_for_server()
        rospy.loginfo("ebot_perception: Perception server is online!")
        self.object = PerceptionGoal()

    def object_recognition(self, ob_name):
        '''A function to send the object name of the object to be picked as a goal to perception action server
            parameter: ob_name: String: name of object to be picked
            return: nothing'''

        self.object = PerceptionGoal()
        self.object.ob_name = ob_name
        self.Perception_client.send_goal(self.object, feedback_cb=self.feedback_cb)
        rospy.loginfo("{} (goal) send for Perception".format(ob_name))
        
    
    def feedback_cb(self, feedback):
        rospy.loginfo('[Result] Object Identified : {}'.format(feedback))

    def object_result(self):
        '''A function to wait for result from action server and receive the result
            parameters: no parameters
            returns: result received from action server: PerceptionResult'''

        rospy.loginfo("Finding object : {}".format( self.object.ob_name))

        # wait for result from perception action server
        self.Perception_client.wait_for_result()

        # get the result from perception action server
        self.ob_result = self.Perception_client.get_result()

        if self.ob_result.ob_success:
            rospy.loginfo('[Result] {} found at : '.format( self.object.ob_name))
            print (self.ob_result)
            return self.ob_result

        else:
            rospy.logerr('{} not found, moving to next location'.format( self.object.ob_name))
            return self.ob_result
...
class TaskManager:
    ...
    def __init__(self, nav_client, perception_client, manipulation_client):
...
def main():
    ...
    # initializing ebot_perception action client
    ebot_perception = PerceptionActionClient()

    # send arm to travel pose befor moving
    ebot_manipulation = Ur5ManipulationClient()

    # initializing ebot_task_manager object
    ebot_task_manager = TaskManager(ebot_navigation, ebot_perception, ebot_manipulation)
